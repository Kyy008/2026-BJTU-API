...
public class CommandExecutor {
    ...
    private String help(String topic) {
        ...
        if ("2".equals(topic) || "上传".equals(topic)) {
            return """
                    上传格式：
                    “上传 公司= 城市= 岗位= 薪资= 学历要求= 公司类型= 岗位类型=“

                    公司、城市、岗位、薪资为必填；学历要求、公司类型、岗位类型为选填。

                    示例：”上传 公司=字节跳动 城市=北京 岗位=后端开发实习生 薪资=300/天 学历要求=本科 岗位类型=实习“""";
        }
        ...
        if ("4".equals(topic) || "更新".equals(topic)) {
            return """
                    更新格式：
                    “更新 编号= 公司= 城市= 岗位= 薪资= 学历要求= 公司类型= 岗位类型=“

                    编号为必填；其他字段均为选填，填哪个就更新哪个。

                    示例：”更新 编号=1 薪资=350/天 学历要求=本科 岗位类型=实习“""";
        }
